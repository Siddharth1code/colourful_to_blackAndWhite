...
def make_grayscale(input_image):
    ...
    print("decoded aray image : ",decode_array_image)
    converted_grayimage = cv2.cvtColor(decode_array_image,cv2.COLOR_RGB2GRAY)
    status,output_image = cv2.imencode('.PNG',converted_grayimage)
    print("status : ",status)
    return output_image 
